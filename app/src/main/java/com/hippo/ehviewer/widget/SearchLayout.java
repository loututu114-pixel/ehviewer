...
public class SearchLayout extends EasyRecyclerView implements CompoundButton.OnCheckedChangeListener,
    ...
    private void setupRippleEffectSafely(Context context) {
        try {
            // 简化实现：直接设置选择器背景，避免Ripple类冲突
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                // Android 5.0+ 使用原生Ripple效果
                mNormalSearchModeHelp.setBackgroundResource(android.R.drawable.btn_default);
            } else {
                // 低版本使用简单的背景
                mNormalSearchModeHelp.setBackgroundResource(android.R.color.transparent);
            }
    ...
    public void formatListUrlBuilder(ListUrlBuilder urlBuilder, String query) throws EhException {
        ...
        switch (mSearchMode) {
            case SEARCH_MODE_NORMAL:
                ...
                if (nsMode == R.id.search_normal_search) {
                    urlBuilder.setMode(ListUrlBuilder.MODE_NORMAL);
                } else if (nsMode == R.id.search_subscription_search) {
