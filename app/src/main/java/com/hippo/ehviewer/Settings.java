...
//        }
    ...
    // 阅读模式预设
    private static final String KEY_GALLERY_READING_MODE_PRESET = "gallery_reading_mode_preset";
    private static final int DEFAULT_GALLERY_READING_MODE_PRESET = 0; // 默认为漫画模式（右到左）
    
    public static int getGalleryReadingModePreset() {
        return getIntFromStr(KEY_GALLERY_READING_MODE_PRESET, DEFAULT_GALLERY_READING_MODE_PRESET);
    }
    
    public static void putGalleryReadingModePreset(int value) {
        putIntToStr(KEY_GALLERY_READING_MODE_PRESET, value);
    }

    private static final String KEY_READING_DIRECTION = "reading_direction";
    private static final int DEFAULT_READING_DIRECTION = GalleryView.LAYOUT_TOP_TO_BOTTOM;
    ...
    private static final boolean DEFAULT_PRELOAD_GALLERY_LIST = true;

    public static boolean getPreloadGalleryList() {
        return getBoolean(KEY_PRELOAD_GALLERY_LIST, DEFAULT_PRELOAD_GALLERY_LIST);
    }

    public static void putPreloadGalleryList(boolean value) {
        putBoolean(KEY_PRELOAD_GALLERY_LIST, value);
    }
    ...
    // ===============================
    // 画廊优化设置 - SpiderQueen增强配置
    // ===============================
    
    public static final String KEY_GALLERY_OPTIMIZATION_ENABLED = "gallery_optimization_enabled";
    public static final String KEY_GALLERY_SMART_CACHE_ENABLED = "gallery_smart_cache_enabled";
    public static final String KEY_GALLERY_PRELOAD_ENABLED = "gallery_preload_enabled";
    public static final String KEY_GALLERY_PRELOAD_COUNT = "gallery_preload_count";
    
    // SpiderQueen增强配置
    public static final String KEY_ENHANCED_MULTI_THREAD_DOWNLOAD = "enhanced_multi_thread_download";
    public static final String KEY_ENHANCED_PRELOAD_IMAGE = "enhanced_preload_image";
    public static final String KEY_ENHANCED_READ_CACHE_SIZE = "enhanced_read_cache_size";
    public static final String KEY_SMART_PERFORMANCE_MODE = "smart_performance_mode";
    
    /**
     * 获取画廊优化总开关状态
     */
    public static boolean getGalleryOptimizationEnabled() {
        return getBoolean(KEY_GALLERY_OPTIMIZATION_ENABLED, false); // 临时禁用优化进行测试
    }
    
    /**
     * 设置画廊优化总开关
     */
    public static void putGalleryOptimizationEnabled(boolean enabled) {
        putBoolean(KEY_GALLERY_OPTIMIZATION_ENABLED, enabled);
    }
    
    /**
     * 获取智能缓存开关状态
     */
    public static boolean getGallerySmartCacheEnabled() {
        return getBoolean(KEY_GALLERY_SMART_CACHE_ENABLED, true); // 默认启用
    }
    
    /**
     * 设置智能缓存开关
     */
    public static void putGallerySmartCacheEnabled(boolean enabled) {
        putBoolean(KEY_GALLERY_SMART_CACHE_ENABLED, enabled);
    }
    
    /**
     * 获取预加载开关状态
     */
    public static boolean getGalleryPreloadEnabled() {
        return getBoolean(KEY_GALLERY_PRELOAD_ENABLED, true); // 默认启用
    }
    
    /**
     * 设置预加载开关
     */
    public static void putGalleryPreloadEnabled(boolean enabled) {
        putBoolean(KEY_GALLERY_PRELOAD_ENABLED, enabled);
    }
    
    /**
     * 获取预加载图片数量
     */
    public static int getGalleryPreloadCount() {
        return getInt(KEY_GALLERY_PRELOAD_COUNT, 3); // 默认预加载3张
    }
    
    /**
     * 设置预加载图片数量
     */
    public static void putGalleryPreloadCount(int count) {
        putInt(KEY_GALLERY_PRELOAD_COUNT, MathUtils.clamp(count, 1, 8)); // 限制1-8张
    }
    ...
    /**
     * 获取增强的多线程下载数量
     * 基于设备性能和网络状况自动调整，提供比默认更激进的配置
     */
    public static int getEnhancedMultiThreadDownload() {
        if (getBoolean(KEY_SMART_PERFORMANCE_MODE, true)) {
            // 智能性能模式：根据设备RAM自动调整
            long totalMemory = Runtime.getRuntime().maxMemory() / 1024 / 1024; // MB
            if (totalMemory >= 6 * 1024) { // 6GB+ 设备
                return getInt(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD, 6);
            } else if (totalMemory >= 4 * 1024) { // 4GB+ 设备
                return getInt(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD, 5);
            } else { // 其他设备
                return getInt(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD, 4);
            }
        } else {
            // 手动模式：用户自定义配置
            return getInt(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD, 5);
     ...
     * 设置增强的多线程下载数量
     */
    public static void putEnhancedMultiThreadDownload(int count) {
        putInt(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD, MathUtils.clamp(count, 1, 10));
    }
     ...
     * 提供比默认更智能的预加载策略
     */
    public static int getEnhancedPreloadImage() {
        if (getBoolean(KEY_SMART_PERFORMANCE_MODE, true)) {
            // 智能性能模式：根据设备能力自动调整
            long totalMemory = Runtime.getRuntime().maxMemory() / 1024 / 1024; // MB
            if (totalMemory >= 12 * 1024) { // 12GB+ 设备
                return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 30);
            } else if (totalMemory >= 8 * 1024) { // 8GB+ 设备
                return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 25);
            } else if (totalMemory >= 6 * 1024) { // 6GB+ 设备
                return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 20);
            } else if (totalMemory >= 4 * 1024) { // 4GB+ 设备
                return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 15);
            } else { // 其他设备
                return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 10);
            }
        } else {
            // 手动模式：用户自定义配置
            return getInt(KEY_ENHANCED_PRELOAD_IMAGE, 15);
     ...
     * 设置增强的预加载图片数量
     */
    public static void putEnhancedPreloadImage(int count) {
        putInt(KEY_ENHANCED_PRELOAD_IMAGE, MathUtils.clamp(count, 0, 50));
    }
     ...
     * 提供比默认更大的缓存配置
     */
    public static int getEnhancedReadCacheSize() {
        if (getBoolean(KEY_SMART_PERFORMANCE_MODE, true)) {
            // 智能性能模式：根据可用存储空间自动调整
            long totalMemory = Runtime.getRuntime().maxMemory() / 1024 / 1024; // MB
            if (totalMemory >= 6 * 1024) { // 6GB+ 设备
                return getInt(KEY_ENHANCED_READ_CACHE_SIZE, 400); // 400MB缓存
            } else if (totalMemory >= 4 * 1024) { // 4GB+ 设备
                return getInt(KEY_ENHANCED_READ_CACHE_SIZE, 320); // 320MB缓存
            } else { // 其他设备
                return getInt(KEY_ENHANCED_READ_CACHE_SIZE, 240); // 240MB缓存
            }
        } else {
            // 手动模式：用户自定义配置
            return getInt(KEY_ENHANCED_READ_CACHE_SIZE, 320);
        }
    }
     ...
     * 设置增强的读取缓存大小
     */
    public static void putEnhancedReadCacheSize(int size) {
        putInt(KEY_ENHANCED_READ_CACHE_SIZE, MathUtils.clamp(size, 40, 1024));
    }
    
    /**
     * 获取智能性能模式状态
     */
    public static boolean getSmartPerformanceMode() {
        return getBoolean(KEY_SMART_PERFORMANCE_MODE, true);
    }
    
    /**
     * 设置智能性能模式
     */
    public static void putSmartPerformanceMode(boolean enabled) {
        putBoolean(KEY_SMART_PERFORMANCE_MODE, enabled);
    }
    ...
    public static boolean isHighPerformanceDevice() {
        long totalMemory = Runtime.getRuntime().maxMemory() / 1024 / 1024; // MB
        return totalMemory >= 4 * 1024; // 4GB+ 设备
    }
    
    /**
     * 重置SpiderQueen增强配置为推荐值
     */
    public static void resetSpiderQueenEnhancedSettings() {
        putBoolean(KEY_SMART_PERFORMANCE_MODE, true);
        // 清除手动配置，使用智能模式的默认值
        sSettingsPre.edit()
            .remove(KEY_ENHANCED_MULTI_THREAD_DOWNLOAD)
            .remove(KEY_ENHANCED_PRELOAD_IMAGE)
            .remove(KEY_ENHANCED_READ_CACHE_SIZE)
            .apply();
    }
}
