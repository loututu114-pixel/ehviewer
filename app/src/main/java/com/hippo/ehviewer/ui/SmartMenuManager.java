...
import com.hippo.ehviewer.ui.gesture.GestureNavigationManager;
import com.hippo.ehviewer.ui.addressbar.SmartAddressBarManager;
...
public class SmartMenuManager {
    ...
    public void createSmartMenu(@NonNull Menu menu) {
        try {
            ...
            menu.add(2, R.id.menu_theme_toggle, 23, "🌙 主题切换")
                .setIcon(android.R.drawable.ic_menu_preferences)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
                
            menu.add(2, R.id.menu_gesture_settings, 24, "👆 手势设置")
                .setIcon(android.R.drawable.ic_menu_preferences)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
                
            menu.add(2, R.id.menu_addressbar_settings, 25, "🔍 地址栏设置")
                .setIcon(android.R.drawable.ic_menu_search)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
    ...
    public boolean handleMenuItemClick(@NonNull MenuItem item) {
        try {
            ...
            // EhViewer特色功能组
                ...
                return true;
            } else if (itemId == R.id.menu_theme_toggle) {
                toggleTheme();
                return true;
            } else if (itemId == R.id.menu_gesture_settings) {
                showGestureSettings();
                return true;
            } else if (itemId == R.id.menu_addressbar_settings) {
                showAddressBarSettings();
                return true;
            }
    ...
    private void openBrowserHome() {
        try {
            Intent intent = new Intent(mContext, YCWebViewActivity.class);
    ...
    private void openSystemSearch() {
        try {
            ...
            // 方法3: 备用 - 打开Google搜索网页
            Intent webSearchIntent = new Intent(mContext, YCWebViewActivity.class);
    ...
    private void toggleTheme() {
        try {
            ...
            ThemeManager.ThemeMode currentMode = themeManager.getCurrentThemeMode();
            ...
            int currentSelection = currentMode.value;
            
            new AlertDialog.Builder(mActivity)
                .setTitle("🎨 选择主题模式")
                .setSingleChoiceItems(themeOptions, currentSelection, (dialog, which) -> {
                    ThemeManager.ThemeMode newMode = ThemeManager.ThemeMode.fromValue(which);
                    themeManager.setThemeMode(newMode);
                    
                    Toast.makeText(mContext, 
                                 "主题已切换至: " + themeManager.getThemeDescription(), 
                                 Toast.LENGTH_SHORT).show();
                    
                    // 延迟关闭对话框，让用户看到变化效果
                    dialog.dismiss();
                    
                    // 如果是WebViewActivity，立即应用主题
                    if (mActivity instanceof YCWebViewActivity) {
                        YCWebViewActivity webActivity = (YCWebViewActivity) mActivity;
                        // 调用WebViewActivity的主题应用方法
                        webActivity.recreate(); // 重新创建Activity以应用新主题
                    }
                })
                .setNeutralButton("详细信息", (dialog, which) -> {
                    showThemeDetails(themeManager);
                })
                .setNegativeButton("取消", null)
                .show();
            ...
            Toast.makeText(mContext, "主题切换失败: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
     ...
     * 显示主题详细信息
     */
    private void showThemeDetails(ThemeManager themeManager) {
        String details = themeManager.getThemeStats() + "\n\n" + 
                        com.hippo.ehviewer.ui.theme.WebViewDarkModeAdapter.getDarkModeStats();
    ...
    private void showGestureSettings() {
        try {
            // 获取WebViewActivity的手势管理器
            GestureNavigationManager gestureManager = null;
            if (mActivity instanceof YCWebViewActivity) {
                YCWebViewActivity webActivity = (YCWebViewActivity) mActivity;
                // gestureManager = webActivity.getGestureNavigationManager();
                // 暂时使用默认创建方式，后续添加getter方法
                Toast.makeText(mContext, "手势设置功能正在开发中...", Toast.LENGTH_SHORT).show();
                return;
            }
            
            Toast.makeText(mContext, "手势功能不可用", Toast.LENGTH_SHORT).show();
                
        } catch (Exception e) {
            Log.e(TAG, "Error showing gesture settings", e);
            Toast.makeText(mContext, "显示手势设置失败: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
     ...
     * 显示地址栏设置对话框
     */
    private void showAddressBarSettings() {
        try {
            // 获取WebViewActivity的地址栏管理器
            SmartAddressBarManager addressBarManager = null;
            if (mActivity instanceof YCWebViewActivity) {
                // 暂时显示开发中提示
                Toast.makeText(mContext, "地址栏设置功能正在开发中...", Toast.LENGTH_SHORT).show();
                return;
            }
            
            Toast.makeText(mContext, "地址栏功能不可用", Toast.LENGTH_SHORT).show();
                
        } catch (Exception e) {
            Log.e(TAG, "Error showing address bar settings", e);
            Toast.makeText(mContext, "显示地址栏设置失败: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
}