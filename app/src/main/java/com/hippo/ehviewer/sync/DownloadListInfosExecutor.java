...
public class DownloadListInfosExecutor {
    ...
    public void executeFilterAndSort(int id) {
        service.execute(() -> {
            ...
            } else if (id == R.id.sort_by_gallery_id_asc || id == R.id.sort_by_gallery_id_desc ||
                      id == R.id.sort_by_create_time_asc || id == R.id.sort_by_create_time_desc ||
                      id == R.id.sort_by_rating_asc || id == R.id.sort_by_rating_desc) {
    ...
    private static void merge(DownloadInfo[] arr, int left, int mid, int right, int sortType) {
        ...
        int k = 0;
        while (i <= mid && j <= right) {
            if (sortType == R.id.sort_by_gallery_id_asc) {
                if (arr[i].gid < arr[j].gid) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
                }
            } else if (sortType == R.id.sort_by_gallery_id_desc) {
                if (arr[i].gid > arr[j].gid) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
                }
            } else if (sortType == R.id.sort_by_create_time_asc) {
                if (arr[i].time < arr[j].time) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
                }
            } else if (sortType == R.id.sort_by_create_time_desc) {
                if (arr[i].time > arr[j].time) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
                }
            } else if (sortType == R.id.sort_by_rating_asc) {
                if (arr[i].rating < arr[j].rating) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
                }
            } else if (sortType == R.id.sort_by_rating_desc) {
                if (arr[i].rating > arr[j].rating) {
                    a[k++] = arr[i++];
                } else {
                    a[k++] = arr[j++];
